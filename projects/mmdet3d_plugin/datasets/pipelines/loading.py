...
@PIPELINES.register_module()
class LoadMultiViewImageDSFromFiles(object):
    ...
    def __call__(self, results):
        ...
        # load depth and pvseg img, without transform
        if 'depth_filename' in results.keys() and 'pvseg_filename' in results.keys():
            depth_fns = results['depth_filename']
            depths = [np.fromfile(name, dtype=np.float32, count=-1).reshape(-1, 3) for name in depth_fns]
            results['depth'] = depths
            pvseg_fns = results['pvseg_filename']
            pvsegs = [Image.open(name) for name in pvseg_fns]
            results['pvseg'] = pvsegs
