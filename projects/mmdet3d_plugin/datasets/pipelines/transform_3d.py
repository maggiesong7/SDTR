...
@PIPELINES.register_module()
class ResizeCropFlipImageDS(object):
    """Random resize, Crop and flip the image
    Args:
        size (tuple, optional): Fixed padding size.
    """
    ...
    def __call__(self, results):
        ...
        for i in range(N):
            ...
            # process depth
            new_depth = self._depth_transform(depths[i],
                                              resize=resize,
            ...
            # process pvseg
            new_pvseg = self._seg_transform(pvsegs[i],
                                            resize=resize,
                                            resize_dims=resize_dims,
                                            crop=crop,
                                            flip=flip,
                                            rotate=rotate)
            new_pvseg = to_tensor(new_pvseg).long() # pvseg to tensor
            bits = torch.pow(2, torch.arange(10)).view(-1, 1, 1)
            new_pvseg = ((new_pvseg & bits) > 0.0).float()
            if 'group_idx' in self.data_aug_conf.keys():
                if len(self.data_aug_conf["group_idx"]) < 10:
                    new_pvseg = new_pvseg[self.data_aug_conf["group_idx"], ...]
            new_pvsegs.append(new_pvseg)
        ...
        results['depth'] = torch.stack(new_depths)
        results['pvseg'] = torch.stack(new_pvsegs)
    ...
    def _seg_transform(self, img, resize, resize_dims, crop, flip, rotate):
        # ida_rot = torch.eye(2)
        # ida_tran = torch.zeros(2)
        # adjust image
