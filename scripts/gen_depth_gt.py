    ...
    for i, cam_key in enumerate(cam_keys):
        ...
        np.concatenate([pts_img[:2, :].T, depth[:, None]],
                       axis=1).astype(np.float32).flatten().tofile(
                           os.path.join(data_root, 'pv_labels/depth',
...
if __name__ == '__main__':
    po = Pool(24)
    # mmcv.mkdir_or_exist(os.path.join('/tmp_hdd', 'depth_trainval'))
    mmcv.mkdir_or_exist(os.path.join(data_root, 'pv_labels/depth'))
