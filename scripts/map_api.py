...
def get_image_and_mask(nusc, sample_token, map_api, layer_names, camera_channel):
    
    np_img, vertices_per_layer, my_lidar_mask = render_map_in_image(nusc, map_api, sample_token,
                                                                    layer_names=layer_names,
                                                                    camera_channel=camera_channel)

    num_classes = NUM_MAP + 1
    total_mask = np.zeros((num_classes, np_img.shape[0], np_img.shape[1]), np.uint8)
...
def render_map_in_image(nusc, map_api, sample_token, layer_names,
    ...
    :param alpha:
    :param patch_radius:
    :param min_polygon_area:
    :param render_behind_cam:
    :param render_outside_im:
    :param verbose:
    :param out_path:
    :return:
    """
                            ...
                            if hasattr(inter, 'geoms'):
                                for temp_poly in inter.geoms:
                                    ...
                                    if apply_lidar_aligment:
                                        ...
                                        for k in range(len(lidar_poly_points)):
                                            ...
                                            # lidar_dist = np.square(points[0,k]-bev_lidar_points[0,:])\
                                            #              +np.square(points[1,k]-bev_lidar_points[1,:])
                                    ...
                                    layer_list.append(np.array(points))
                                    # label = layer_name
                                    # ax.add_patch(descartes.PolygonPatch(polygon_proj, fc=color_map[layer_name],
                                ...
                                if apply_lidar_aligment:
                                    ...
                                    if len(heights_list) > 0:
                                        ...
                                        selected_lidars = np.array(selected_lidars_list)
                                        for k in range(points.shape[1]):
                                            lidar_dist = np.square(points[0, k]-selected_lidars[:, 0])\
                                                         + np.square(points[1, k]-selected_lidars[:, 1])
                                            nearest_lidar = np.argmin(lidar_dist)
                                            selected_height = heights_list[nearest_lidar]
                                            heights[:, k] = selected_height
                                    else:
